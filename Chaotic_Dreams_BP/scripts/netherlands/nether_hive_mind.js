...
*/
const HM=Object.freeze({enabled:true,typeHive:"netherlands:hive_mind",typeAtlas:"netherlands:portal_atlas",tagPortal:"nc_portal",tagAnger:"nc_anger:",tagR:"nc_r:",block:"netherlands:fragile_obsidian",minRadius:4,lifeTicks:600,stepEvery:3,placeCap:72,removeCap:48,maxPlaced:5200,damage:3,damageEvery:12,maxTendrils:3,thickAt:55,thickSmall:2,thickBig:3,targetRange:96,targetsEvery:10,maxTargets:48,staggerSpawn:6,staggerEnd:20,scanEvery:40,scanDims:["minecraft:overworld","minecraft:nether","minecraft:the_end"]});
...
function spawnForHive(e){if(!HM.enabled||!e)return;let d;try{d=e.dimension}catch{return}
  const atlas=nearestAtlas(d,e.location,256);if(!atlas)return;
...
function scanForHives(){for(const id of HM.scanDims){let d;try{d=world.getDimension(id)}catch{continue}
    let hs=[];try{hs=d.getEntities({type:HM.typeHive})??[]}catch{continue}
    for(const h of hs){if(!h)continue;const hid=h.id; if(ACTIVE.has(hid))continue;spawnForHive(h)}
  }}

function tick(){if(!HM.enabled)return;const t=system.currentTick|0;if((t%HM.scanEvery)===0)scanForHives();if(!ACTIVE.size)return;let placeBudget=HM.placeCap,removeBudget=HM.removeCap;let players=[];try{players=world.getPlayers({gameMode:GameMode.survival})}catch{try{players=world.getPlayers().filter(isSurvival)}catch{players=[]}}
  for(const[id,s]of ACTIVE){const d=s.dim;let alive=true;try{alive=!(typeof s.e.isValid==="function"&&!s.e.isValid())}catch{alive=false}
    ...
    if(!s.dead&&t-s.ref>=20){let ok=true;try{ok=!(typeof s.atlas.isValid==="function"&&!s.atlas.isValid())}catch{ok=false}
      if(!ok){const a=nearestAtlas(d,s.center,256);if(a)s.atlas=a;else s.dead=true}
